...
#+LATEX_HEADER: \title{\textbf{Researh Proposal \\
...
but symbolic regression completely bypasses this issue.
...
# TODO: New transition
The space of possible mathematical expressions is arbitrarily large, and therefore it is not viable to explore it exhaustively.
...
Under these methods we can encode prior information about the components and the structure of mathematical expressions.
...
Where \(\Psi_i\) is the \(i\)th symbolic tree that represents a function of the features \(x\).
